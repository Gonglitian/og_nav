...
class OGMGenerator:
    ...
    def __init__(self, resolution: float) -> None:
        ...
        try:
            ...
            self.generator = lazy.omni.isaac.occupancy_map.bindings._occupancy_map.Generator(
                physx, stage_id
            )
            
            # Configure generator settings
            # Values 4, 5, 6 represent available, occupied, unknown respectively
            self.generator.update_settings(resolution, 4, 5, 6)
            self.resolution = resolution
            
            print(f"OGM Generator initialized with resolution: {resolution}")
