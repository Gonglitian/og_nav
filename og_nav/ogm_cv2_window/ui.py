...
class OGMScrollUI:
    ...
    def init_sliders(self) -> None:
        ...
        try:
            for key, (init_val, min_val, max_val) in self.SLIDER_PARAMS.items():
                cv2.createTrackbar(
                    key, self.WINDOW_NAME, init_val - min_val, max_val - min_val, lambda x: None
                )
    ...
    def generate_map(self, params: Dict[str, float]) -> None:
        ...
        try:
            ...
            # Generate map
            grid_map = self.generator.generate_grid_map(center, lower, upper, return_img=True)
    ...
    def _prepare_display_image(
        ...
        # Resize image
        resized_map = cv2.resize(grid_map, (new_width, new_height), interpolation=cv2.INTER_NEAREST)
        ...
        cv2.putText(
            display_img, f"Map Size: {width}x{height} (scaled {scale_factor}x)",
            (10, info_y_start), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1
        )
        cv2.putText(
            display_img, f"Center: ({center[0]:.1f}, {center[1]:.1f}, {center[2]:.1f})",
            (10, info_y_start + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1
        )
        cv2.putText(
            display_img, f"Range: ({lower[0]:.1f},{lower[1]:.1f},{lower[2]:.1f}) to ({upper[0]:.1f},{upper[1]:.1f},{upper[2]:.1f})",
            (10, info_y_start + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1
        )
        cv2.putText(
            display_img, "Controls: SPACE=Generate, R=Reset, Q=Quit",
            (10, info_y_start + 75), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1
        )
    ...
    def _show_error_message(self, error_msg: str) -> None:
        ...
        cv2.putText(
            error_img, "Map Generation Error:", 
            (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2
        )
        cv2.putText(
            error_img, error_msg[:50], 
            (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1
        )
        cv2.putText(
            error_img, "Check parameters and try again", 
            (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 1
        )
